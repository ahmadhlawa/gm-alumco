...
export function AdminWebsiteHero() {
  ...
  const handleSave = () => {
    ...
    setTimeout(() => {
      ...
      setToast({
        show: true,
        msg: 'تم تحديث معاينة الواجهة محلياً بنجاح.',
      });
  ...
  const resetToDefault = () => {
    if (confirm('هل أنت متأكد من رغبتك في إعادة تعيين الواجهة للمحتوى الافتراضي؟')) {
      ...
      setToast({
        show: true,
        msg: 'تمت إعادة المعاينة إلى المحتوى الافتراضي.',
      });
    ...
    <div className="space-y-6 text-right" dir="rtl">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-brand-silver mt-1">تعديل العناوين والشارات والأجهزة الترويجية التي تشكّل الانطباع الأول لشركة T.A.S.</p>
        </div>
