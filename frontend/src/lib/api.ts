...
import { toPartnerView, toProductionProjectView, toProjectView, toServiceView, toTestimonialView } from '@/api/adapters';
import { listPartners } from '@/api/partners';
import { listProductionProjects } from '@/api/productionProjects';
...
export const getProductionProjects = async (locale: Locale = 'he') => (await listProductionProjects()).map((item) => toProductionProjectView(item, locale));
